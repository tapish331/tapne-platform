...
export class ProfileService {
  ...
  constructor(private readonly env: NodeJS.ProcessEnv = process.env, blockProvider?: BlockProvider) {
  ...
  getByUsernameForViewer(username: string, viewerId: string): ProfileRecord | undefined {
    const found = this.byUsername.get(normalizeUsername(username));
    if (!found) return undefined;
    if (this.blockProvider && viewerId) {
      const blocked = this.blockProvider.isBlocked(viewerId, found.userId) || this.blockProvider.isBlocked(found.userId, viewerId);
      if (blocked) return undefined;
    }
    return { ...found };
  }

  // Internal helper for search: list all profiles
  __unsafe_all(): ProfileRecord[] {
    return Array.from(this.byUser.values()).map((p) => ({ ...p }));
  }
}

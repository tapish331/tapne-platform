...
export class BookmarkService {
  ...
  private ensureValid({ userId, tripId }: BookmarkPair): void {
  ...
  bookmark(input: BookmarkPair): void {
  ...
  unbookmark(input: BookmarkPair): void {
    this.ensureValid(input);
    const k = this.key(input.userId, input.tripId);
    if (!this.pairs.has(k)) return; // idempotent
    this.pairs.delete(k);
  ...
  isBookmarked(userId: string, tripId: string): boolean {
